...
// Hoisted to module scope: previously declared inside ProfileCreate()'s
// body, which gave it a new component identity on every render (including
// every keystroke) and forced React to remount its subtree.
...
function Step1AboutYou({ direction, formData, errors, updateField, handleNext }: Step1AboutYouProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div>
        ...
        <ErrorMessage errors={errors} field="location" />
      </div>
      ...
      <div>
        ...
        <ErrorMessage errors={errors} field="rate" />
      </div>
...
interface Step2SkillsPortfolioProps {
  direction: 'forward' | 'backward';
  formData: FormData;
  errors: FormErrors;
  skillInput: string;
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          <motion.button
            ...
            className="px-4 py-2.5 bg-[#C6FF34] text-black rounded-xl text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed inline-flex items-center gap-1"
            whileTap={{ scale: 0.95 }}
          >
            <Plus size={16} />
            Add
        ...
        <ErrorMessage errors={errors} field="skills" />
...
// ─── Step 3: Photo & Review ────────────────────────────────
interface Step3PhotoReviewProps {
  direction: 'forward' | 'backward';
  formData: FormData;
  errors: FormErrors;
  updateField: <K extends keyof FormData>(field: K, value: FormData[K]) => void;
