...
## 2026-07-31 — Wired talent profiles to the real backend (create, list, view, edit)

**Decision:** before doing a manual production walkthrough, read the actual code paths first — and found that `talent.createOwnProfile`/`updateOwnProfile`/`getOwnProfile`, despite being real, tested, registered backend endpoints, were called by **zero** frontend code. `Onboarding.tsx`, `ProfileCreate.tsx`, and `Dashboard.tsx`'s "My Profile" link all wrote straight to `localStorage` instead. `TalentDirectory.tsx` and `TalentProfile.tsx` went further: they never called the backend at all, not even the plain public `talent.list`/`getById` reads — which, on inspection, weren't even registered on `appRouter`. `CLAUDE.md`'s claim that talent profiles are "backed by a real database and API" was true of the backend in isolation only.

**What shipped:**
- `server/router.ts`: registered `talent.list` and `talent.getById` (plain `publicProcedure` reads, no auth, no known issues — unlike the genuinely quarantined routers, these were just never wired in). `delete` stays unregistered; nothing calls it.
- `src/hooks/useOwnTalentProfile.ts` (new): single shared hook wrapping `getOwnProfile`/`createOwnProfile`/`updateOwnProfile` into one `save()` call that picks create vs. update — used by both pages that write a profile, instead of each hand-rolling that logic.
- `Onboarding.tsx`: its final step now calls the real `save()` with the name/bio/category/skills/location it already derives from the wizard, instead of fabricating a `localStorage`-only profile object. The richer Levav 28™/WRI personalization fields (confidence sliders, motivation, career challenge) stay local — intentionally, they have no backend model yet, matching the rest of this prototype's documented split.
- `ProfileCreate.tsx`: now the real edit surface — pre-fills from the real profile when one exists (falling back to onboarding data, then blank), calls the same `save()`, and its copy now honestly reflects state ("Edit Your Profile" / "Save Changes" vs. "Create Your Profile" / "Create My Profile"). `rate`/`portfolio`/`avatar` have no column in `db/schema.ts` yet, so they're kept as a local supplement under a dedicated `talent_profile_extras` key — not silently dropped, not faked as backend-real either.
- `TalentDirectory.tsx` / `TalentProfile.tsx`: now merge real `talent.list`/`getById` results with the existing `MOCK_TALENTS` seed data (same merge pattern already used by QuickWork/Impact/Feed). `TalentProfile.tsx` routes by whether the id in the URL is a UUID (real) or a small mock id, so both `/talent/:realUuid` and the existing mock detail pages keep working.

**Bugs found and fixed along the way, not just worked around:**
- `server/trpc.ts`'s `authedMiddleware`/`adminMiddleware` threw a plain `Error('UNAUTHORIZED'/'FORBIDDEN')` instead of `TRPCError`, which tRPC reports to the client as a 500 Internal Server Error instead of a proper 401/403. Caught because `TalentDirectory.tsx`'s unconditional `getOwnProfile` call was the first real guest-visitor exercise of an authed procedure; fixed for every authed/admin procedure, not just this call site. Frontend calls to `getOwnProfile` are now also gated on `isAuthenticated` so a guest doesn't attempt an authed call at all.
- A missing `useMemo` dependency in my own first pass at `TalentDirectory.tsx` (real query data loaded but the merged list never recomputed) — caught by a real Playwright run against the live local stack, not just typecheck, and fixed before this was reported as done.

**Verified, not just typechecked:** a full local run against the real database (Vite + standalone Hono server + local Postgres) — registered through Onboarding, confirmed the row via direct SQL, confirmed it appears in `/talent`, confirmed `/talent/:uuid` renders it, then logged in and confirmed `ProfileCreate.tsx` pre-fills from that same row, edited it, and confirmed via SQL that it updated in place (no duplicate row). 33/33 tests passing (`server/router.test.ts` allowlist updated for `talent.list`/`getById`), typecheck at the same 160-error pre-existing baseline, production build succeeds.

**What this does NOT do:** `Dashboard.tsx`'s own display of "your profile" (beyond the `/profile/create` link) wasn't touched — out of scope for this pass, a smaller natural follow-up. `rate`/`portfolio`/`avatar` remain local-only until `db/schema.ts` grows columns for them, which is a schema-migration decision, not something to smuggle in here.

**Next decision point:** whether to extend `talents` schema for rate/portfolio/avatar, and which of the previously-identified backend gaps (Feed, quarantined routes) to take on next.
