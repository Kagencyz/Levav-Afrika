...
- **Tenant/role model:** introduce `organizations` and `organization_members` tables so "employer" is modeled as an organization with multiple member-users, not a 1:1 user role; narrow `users.role` to `talent | employer_member | admin`; model Champion as an earned status, not a role value. Full entity definitions in `docs/DOMAIN_MODEL.md`.
