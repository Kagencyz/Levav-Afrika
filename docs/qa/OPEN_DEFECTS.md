...
| F-06 | Taxonomy tests mock database execution, so the migration SQL itself is not exercised against a real Postgres | No database is available in the review environment and the packet did not require integration tests. The logic, authorisation and PDR-0016 behaviour are all covered | **Run the migration and rollback against a real Supabase instance before WP-0102 consumes the taxonomy.** Cheap now, expensive after onboarding depends on it |
