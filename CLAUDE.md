...
- **Deployment: Vercel. Cloudflare is DNS only.** `levavtalent.com` is fronted by Cloudflare for DNS/CDN and served by Vercel — confirmed by the repository owner, 2026-08-12. `vercel.json` supplies `/api/*` routing and the SPA fallback, and `api/index.ts` is bound to Vercel's Node runtime (`@hono/node-server/vercel`, needed for `pg`'s raw TCP sockets). The repository contains **no** Cloudflare Pages infrastructure — no `_redirects`, `functions/`, `_worker.js`, `wrangler` or `@cloudflare/*`. A **Cloudflare Pages project (`levav`) builds this repository on every push and is not the production path**; do not read it as one, and do not "fix" the missing Cloudflare config. It publishes to `levav.pages.dev`, including a stable per-branch URL. Because Pages has no handler for `/api/*`, **that deployment serves the frontend with no backend** — auth and every tRPC call fail there, while the localStorage and `MOCK_*` surfaces still render. It therefore looks far more functional than it is. **Never evaluate product behaviour, demo, or accept a Work Packet against a `pages.dev` URL.** Because Vercel's build command is `npm run build` (`typecheck && vite build`), the typecheck gate does protect production — which is why WP-0002 item 7 must not be landed before the WP-0001 deletions.
